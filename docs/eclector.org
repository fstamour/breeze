...
+------------------------------------+--------------------------------------------------------+
|  doesn't discard any information   |     discard all whitespaces, comments and feature      |
|                                    |      expressions that fails (e.g. ~#+(or) this~)       |
+------------------------------------+--------------------------------------------------------+
| will never signal an error (unless |  signals error and provide restarts to let the client  |
|          there's a bug!)           |           decide how to handle the situation           |
+------------------------------------+--------------------------------------------------------+
|  returns nested vector and object  |           returns cons (just like ~cl:read~)           | 
+------------------------------------+--------------------------------------------------------+
|     does not evaluate anything     |         evaluates #. #+ #- and interns symbols         |
+------------------------------------+--------------------------------------------------------+
...
* Eclector's Concrete Syntax Tree
:PROPERTIES:
